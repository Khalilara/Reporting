...
public interface PreparedDataRepository extends JpaRepository<PreparedData, Long> {
        ...
        @Query("SELECT DISTINCT p.reseller FROM PreparedData p WHERE p.secondReseller IS NULL OR p.secondReseller = '(empty)'OR p.secondReseller = '(Empty)'")
